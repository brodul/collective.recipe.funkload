...
class FunkloadWrapper(object):
    ...
    def __init__(self, url, buildout_dir, data_dir,
            report_dir, record_dir, record_name, record_port):
        self._url = url
    ...
    def _dispatch(self):
        ...
        if action and action in actions:
            if len(self._args[2:]):  # if argument don t filter path
                ...
                test_path = ['--test-path=%s' % (path)
                    for path in sys.path
                    if path.startswith(self._dir) and path != self._dir
                ]

            self._injected_args = [self._args[0]] +\
                test_path +\
                ['--url=%s' % self._url] +\
                self._args[2:]
    ...
    def report(self, no_args=False):
        ...
        if len(self._clean_args) > 1 and not no_args:
            print "Found extra command line arguments,\
            passing command through unadjusted"
    ...
    def record(self):
        ...
        FunkLoad script or generates a FunkLoad unit test
        if test_name is specified.
        """
...
def main(url, buildout_dir, report_destination, data_destination,
    record_output, record_test_name, record_proxy_port):
    # XXX WTF
    wrapper = FunkloadWrapper(url, buildout_dir, data_destination,
        report_destination, record_output,
        record_test_name, record_proxy_port)
    wrapper._dispatch()
