...
class TestRunner(object):
    ...
    def __init__(self, buildout, name, options):
        ...
        default_report_destination = os.path.join(
            self.buildout['buildout'].get('directory'),
            'var',
            'funkload',
            'reports'
        ...
        self.report_destination = self.options.get(
            'report_destination',
            default_report_destination
        )

        self.record_output = self.options.get(
            'record_output',
            default_record_output
        )

        self.record_test_name = self.options.get(
            'record_test_name',
            'SimpleRecord'
        )
        ...
        eggs = self.options.get(
            'eggs',
            'funkload\ncollective.funkload\ncollective.recipe.funkload'
        )

        required_eggs = ['funkload',
            'collective.funkload',
            'collective.recipe.funkload'
        ]
